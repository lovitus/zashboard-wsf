...
## [1.0.3-test.0](https://github.com/lovitus/zashboard-wsf/releases/tag/v1.0.3-test.0) (2026-04-06)

### Native Release Summary

Test release for the built-in UI shell refresh and setup flow hardening.

Built-in upstream UI baseline for this release: `zashboard 2.7.0`.

### Highlights

- redesign the built-in shell, page headers, section cards, side navigation, and toolbar structure for a more deliberate desktop/mobile layout
- refresh setup and settings pages so backend management, built-in recovery paths, and configuration sections are easier to scan
- improve empty states, dialog affordances, and card density across overview, proxies, connections, logs, and rules
- harden setup behavior by preserving unrelated query parameters, validating port ranges, and preventing delete-confirm Enter from triggering a backend submit
- fix the custom build wrapper so release builds now wait for the real Vite exit status before reporting success

### Test Scope

- this is a prerelease intended to validate UI structure, release automation, and native packaging behavior before the next stable tag
- release assets should be checked on Windows, macOS Apple Silicon, and Android with extra attention on setup, settings scroll behavior, mobile docks, and sidebar interactions

### Notes

- stable users should remain on `v1.0.2` until this shell refresh line is validated
- the built-in upstream UI baseline is unchanged in this test release; the main change is the surrounding native/built-in experience
